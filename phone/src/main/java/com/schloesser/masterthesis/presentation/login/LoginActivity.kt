            ...
            ApiFactory.getInstance(this).api.login(
                ...
                override fun onResponse(
                    ...
                    if (response.code() == 200) {
                        SessionRepository.getInstance(this@LoginActivity).accessToken =
                            response.body()?.token?.access!!
                        SessionRepository.getInstance(this@LoginActivity).refreshToken =
                            response.body()?.token?.refresh!!
                        startActivity(Intent(this@LoginActivity, InternalActivity::class.java))
                        finish()
                    } else {
                        Toast.makeText(
